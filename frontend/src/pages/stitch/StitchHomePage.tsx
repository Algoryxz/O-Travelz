import React, { useState, useMemo } from 'react';
import type { StitchTab } from '../../components/stitch/StitchNavbar';
import { useLocation } from '../../context/LocationContext';
import { useRegisterAIContext } from '../../context/AIContext';
import { MANUAL_IMAGE_OVERRIDES } from '../../utils/imageRegistry';
import { getFeaturedOdishaDestinations, getPlaceImageUrl } from '../../utils/imageService';
import { CoverflowCarousel, type CoverflowItem } from '../../components/gallery/CoverflowCarousel';
import { SurpriseMeButton } from '../../components/discovery/SurpriseMeButton';
import { ImageIdentifyButton } from '../../components/discovery/ImageIdentifyButton';
import { CircuitsTicker } from '../../components/home/CircuitsTicker';
import { EssentialsSection } from '../../components/home/EssentialsSection';
import { Heritage3DSection } from '../../components/home/Heritage3DSection';
import { OdishaDestinationWorlds } from '../../components/destination/OdishaDestinationWorlds';
import { TiltCard } from '../../components/ui/TiltCard';
import { Box, Film, Compass, CalendarDays, Sparkles, Navigation, ArrowRight } from 'lucide-react';

interface StitchHomePageProps {
  onNavigate: (tab: StitchTab, params?: Record<string, string>) => void;
  onSearch: (query: string) => void;
  onOpenOnboarding?: () => void;
}

export const StitchHomePage: React.FC<StitchHomePageProps> = ({
  onNavigate,
  onSearch,
  onOpenOnboarding,
}) => {
  const [searchInput, setSearchInput] = useState('');
  const [selectedImageData, setSelectedImageData] = useState<string | null>(null);
  const [selectedImageName, setSelectedImageName] = useState<string | null>(null);
  const { locationName, isLive } = useLocation();

  useRegisterAIContext(
    useMemo(
      () => ({
        page: 'home',
        location: {
          city: locationName,
          district: locationName,
          location_type: isLive ? 'LIVE_GPS' : 'USER_SELECTION',
        },
      }),
      [locationName, isLive]
    )
  );

  const discoveryCarouselItems: CoverflowItem[] = useMemo(() => {
    const featured = getFeaturedOdishaDestinations();
    return featured.map((d) => ({
      id: d.id,
      title: d.name,
      src: d.imageUrl,
      imageUrl: d.imageUrl,
      alt: d.name,
      category: d.category,
      tag: d.category,
      subtitle: d.description,
      description: d.description,
      meta: d.location,
      location: d.location,
    }));
  }, []);

  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchInput.trim()) {
      onSearch(searchInput.trim());
      onNavigate('destinations', { query: searchInput.trim() });
    } else {
      onNavigate('destinations');
    }
  };

  const heroDestinations = useMemo(() => [
    {
      id: "place_konark_001",
      name: "Konark Sun Temple",
      odiaName: "କୋଣାର୍କ ସୂର୍ଯ୍ୟ ମନ୍ଦିର",
      category: "HERITAGE · UNESCO",
      location: "Konark, Puri District",
      detail: "13th Century Solar Chariot & 3D Model",
      has3D: true,
      hasVideo: true,
      imageUrl: getPlaceImageUrl("place_konark_001"),
    },
    {
      id: "place_puri_001",
      name: "Jagannath Temple & Puri Coast",
      odiaName: "ଶ୍ରୀ ଜଗନ୍ନାଥ ମନ୍ଦିର",
      category: "SACRED HERITAGE",
      location: "Puri Golden Coast",
      detail: "12th Century Sacred Shikhara",
      has3D: true,
      hasVideo: true,
      imageUrl: getPlaceImageUrl("place_puri_001"),
    },
    {
      id: "place_chilika_001",
      name: "Chilika Marine Lagoon",
      odiaName: "ଚିଲିକା ହ୍ରଦ",
      category: "COASTAL WETLAND",
      location: "Satapada & Mangalajodi",
      detail: "Asia's Largest Brackish Lagoon & Dolphins",
      has3D: true,
      hasVideo: true,
      imageUrl: getPlaceImageUrl("place_chilika_001"),
    },
    {
      id: "place_daringbadi_001",
      name: "Daringbadi Valleys",
      odiaName: "ଦାରିଙ୍ଗବାଡ଼ି",
      category: "EASTERN GHATS",
      location: "Kandhamal Pine Valleys",
      detail: "Misty Valleys & Coffee Estates",
      has3D: false,
      hasVideo: true,
      imageUrl: getPlaceImageUrl("place_daringbadi_001"),
    },
  ], []);

  const [activeHeroIndex, setActiveHeroIndex] = useState(0);

  // Auto-cycle hero slides every 7 seconds
  React.useEffect(() => {
    const timer = setInterval(() => {
      setActiveHeroIndex((prev) => (prev + 1) % heroDestinations.length);
    }, 7000);
    return () => clearInterval(timer);
  }, [heroDestinations.length]);

  return (
    <div className="w-full min-h-screen flex flex-col bg-[#FBF9F5]">
      {/* 1. CINEMATIC ODISHA HERO SECTION WITH 3D & VIDEO ACCENTS */}
      <section className="relative w-full min-h-[92vh] lg:min-h-[90vh] flex flex-col justify-center pt-28 pb-16 px-6 md:px-12 overflow-hidden bg-[#12161E]">
        {/* Background Crossfading Photography */}
        {heroDestinations.map((dest, idx) => (
          <img
            key={dest.id}
            src={dest.imageUrl}
            alt={dest.name}
            className={`absolute inset-0 w-full h-full object-cover object-center z-0 transition-all duration-1000 ease-out ${
              idx === activeHeroIndex ? "opacity-100 scale-100" : "opacity-0 scale-105 pointer-events-none"
            }`}
            onError={(e) => {
              (e.currentTarget as HTMLImageElement).src = getPlaceImageUrl("place_konark_001");
            }}
          />
        ))}

        {/* Rich Atmospheric Vignette Overlays */}
        <div className="absolute inset-0 bg-gradient-to-t from-[#12161E] via-[#12161E]/65 to-[#12161E]/35 z-10 pointer-events-none" />
        <div className="absolute inset-0 bg-gradient-to-r from-[#12161E]/95 via-[#12161E]/60 to-transparent z-10 pointer-events-none" />

        {/* Ambient Color Glow */}
        <div className="absolute top-1/4 left-10 w-96 h-96 bg-[#0D5C3A]/25 rounded-full blur-[140px] pointer-events-none z-10" />
        <div className="absolute bottom-10 right-10 w-96 h-96 bg-[#C69214]/20 rounded-full blur-[140px] pointer-events-none z-10" />

        {/* Hero Grid Container */}
        <div className="relative z-20 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
          
          {/* Left Column: Headline, Copy & Actions */}
          <div className="lg:col-span-7 space-y-6 text-center md:text-left">
            <div className="inline-flex items-center gap-2 bg-[#0D5C3A]/80 backdrop-blur-md border border-white/20 px-3.5 py-1.5 rounded-full text-white text-xs font-mono mb-1 shadow-md">
              <span className="w-2 h-2 rounded-full bg-[#C69214] animate-pulse" />
              <span>O-Travelz V3 • Smart Tourism Intelligence</span>
              {isLive && (
                <span className="text-[#E5DFD5]">| Near {locationName}</span>
              )}
            </div>

            <h1 className="text-4xl sm:text-6xl lg:text-7xl font-display font-bold text-white tracking-tight leading-[1.08]">
              Explore Odisha.<br />
              <span className="italic font-normal text-[#C69214]">Experience it. Plan it.</span>
            </h1>

            <p className="text-base sm:text-lg font-body text-[#E5DFD5] max-w-2xl leading-relaxed">
              Immerse in ancient stone sanctuaries, pristine golden coastlines, and living traditions through 161 verified destinations, video previews, and interactive 3D models.
            </p>

            {/* Quick Action Navigation Pills */}
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2.5 pt-1">
              <button
                type="button"
                onClick={() => onNavigate('map', { mode: 'destinations' })}
                className="px-4 py-2 rounded-xl bg-[#0D5C3A] hover:bg-[#0A472C] text-white text-xs font-semibold flex items-center gap-1.5 backdrop-blur-md border border-white/20 shadow-md transition-all cursor-pointer"
              >
                <Navigation className="w-3.5 h-3.5 text-[#C69214]" />
                <span>Explore Nearby</span>
              </button>

              <button
                type="button"
                onClick={() => onNavigate('plan')}
                className="px-4 py-2 rounded-xl bg-white/15 hover:bg-white/25 text-white text-xs font-semibold flex items-center gap-1.5 backdrop-blur-md border border-white/25 shadow-md transition-all cursor-pointer"
              >
                <CalendarDays className="w-3.5 h-3.5 text-[#C69214]" />
                <span>Plan My Trip</span>
              </button>

              <button
                type="button"
                onClick={() => {
                  const el = document.getElementById('heritage-3d-section');
                  el?.scrollIntoView({ behavior: 'smooth' });
                }}
                className="px-4 py-2 rounded-xl bg-[#C69214] hover:bg-[#B87B22] text-white text-xs font-semibold flex items-center gap-1.5 shadow-md transition-all cursor-pointer"
              >
                <Box className="w-3.5 h-3.5" />
                <span>3D Heritage Tour</span>
              </button>
            </div>

            {/* Editorial Search Pod with Image Scan & Surprise Me Buttons */}
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 max-w-3xl pt-2">
              <form
                onSubmit={handleSearchSubmit}
                className="flex-1 bg-white/95 backdrop-blur-xl border border-white/40 p-2 sm:p-2.5 rounded-2xl shadow-2xl flex items-center gap-2"
              >
                <div className="flex-1 flex items-center gap-2.5 px-3 min-w-0">
                  <span className="material-symbols-outlined text-[#0D5C3A] text-xl shrink-0">search</span>
                  <input
                    type="text"
                    placeholder="Search temples, beaches, waterfalls, food..."
                    value={searchInput}
                    onChange={(e) => setSearchInput(e.target.value)}
                    className="w-full bg-transparent text-[#12161E] placeholder-[#70798B] font-body text-xs sm:text-sm focus:outline-none truncate"
                  />
                </div>

                {/* Camera Image Scan / Upload Button [ 📷 ] */}
                <ImageIdentifyButton
                  onImageSelected={(base64, name) => {
                    setSelectedImageData(base64);
                    setSelectedImageName(name);
                    onNavigate('destinations', { query: name || 'Konark' });
                  }}
                />

                {/* Surprise Me Dice Button [ 🎲 ] */}
                <SurpriseMeButton
                  variant="hero"
                  onNavigateToMap={(id, lat, lon) => {
                    onNavigate('map', { placeId: id, lat: lat ? String(lat) : '', lon: lon ? String(lon) : '' });
                  }}
                  onPlanTrip={(name) => {
                    onNavigate('plan', { query: name });
                  }}
                />

                <button
                  type="submit"
                  className="bg-[#0D5C3A] hover:bg-[#0A472C] text-white px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl font-body text-xs sm:text-sm font-semibold transition-all duration-300 shadow-md hover:shadow-lg flex items-center justify-center gap-1.5 shrink-0 cursor-pointer"
                >
                  <span>Explore</span>
                  <ArrowRight className="w-4 h-4 text-[#C69214]" />
                </button>
              </form>
            </div>
          </div>

          {/* Right Column: 4 Spatial Glass Destination Panels */}
          <div className="lg:col-span-5 relative mt-6 lg:mt-0 space-y-3">
            {/* Header label */}
            <div className="flex items-center justify-between text-xs text-[#E5DFD5]/90 font-mono tracking-widest uppercase px-1">
              <span className="flex items-center gap-1.5 font-semibold text-[#C69214]">
                <Sparkles className="w-3.5 h-3.5" />
                <span>FEATURED ODYSSEYS</span>
              </span>
              <span className="text-[#C69214] font-semibold">
                {String(activeHeroIndex + 1).padStart(2, "0")} / 04
              </span>
            </div>

            {/* Spatial Glass Stack Panels */}
            <div className="flex flex-col space-y-2.5 sm:space-y-3 relative">
              {heroDestinations.map((dest, idx) => {
                const isActive = idx === activeHeroIndex;
                return (
                  <div
                    key={dest.id}
                    onClick={() => setActiveHeroIndex(idx)}
                    className={`w-full text-left p-3.5 sm:p-4 rounded-2xl transition-all duration-500 cursor-pointer flex items-center justify-between gap-3.5 backdrop-blur-[24px] backdrop-saturate-[135%] border select-none ${
                      isActive
                        ? "bg-white/[0.22] border-white/60 border-l-4 border-l-[#C69214] shadow-[0_16px_40px_rgba(0,0,0,0.5)] lg:translate-x-3 scale-[1.02] z-20"
                        : "bg-white/[0.12] hover:bg-white/[0.18] border-white/25 hover:border-white/40 shadow-[0_8px_24px_rgba(0,0,0,0.3)] scale-[0.98] opacity-80 hover:opacity-100 z-10"
                    }`}
                  >
                    <div className="flex items-center gap-3.5 min-w-0">
                      {/* Destination Thumbnail with Glass Ring */}
                      <div className="relative w-12 h-12 rounded-xl overflow-hidden shadow-md shrink-0 ring-1 ring-white/30">
                        <img
                          src={dest.imageUrl}
                          alt={dest.name}
                          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                        />
                        <div className={`absolute inset-0 ${isActive ? "bg-[#0D5C3A]/20" : "bg-black/20"}`} />
                        {isActive && (
                          <div className="absolute top-1 right-1 w-2 h-2 rounded-full bg-[#C69214] ring-2 ring-white animate-pulse" />
                        )}
                      </div>

                      {/* Editorial Destination Info */}
                      <div className="min-w-0 space-y-0.5">
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-mono font-semibold tracking-wider text-[#C69214] uppercase truncate">
                            {dest.category}
                          </span>
                          {dest.has3D && (
                            <span className="text-[9px] font-mono bg-[#0D5C3A] text-white px-1.5 py-0.2 rounded-md font-semibold shrink-0">
                              3D
                            </span>
                          )}
                        </div>
                        <h4 className="font-display font-bold text-base sm:text-lg text-white leading-tight truncate">
                          {dest.name}
                        </h4>
                        <p className="text-[11px] font-body text-[#E5DFD5]/90 truncate">
                          {dest.location}
                        </p>
                      </div>
                    </div>

                    {/* Action CTA */}
                    <div className="shrink-0 flex items-center gap-1">
                      {isActive ? (
                        <button
                          type="button"
                          onClick={(e) => {
                            e.stopPropagation();
                            onNavigate('destinations', { query: dest.name });
                          }}
                          className="px-3 py-1.5 rounded-lg bg-[#0D5C3A] hover:bg-[#0A472C] text-white text-xs font-semibold flex items-center gap-1 transition-all shadow-xs cursor-pointer whitespace-nowrap"
                        >
                          <span>Explore</span>
                          <ArrowRight className="w-3.5 h-3.5 text-[#C69214]" />
                        </button>
                      ) : (
                        <span className="material-symbols-outlined text-white/50 text-base">
                          chevron_right
                        </span>
                      )}
                    </div>
                  </div>
                );
              })}
            </div>
          </div>
        </div>
      </section>

      {/* 2. CINEMATIC DESTINATION PREVIEW: ODISHA DESTINATION WORLDS */}
      <div id="destination-worlds-section">
        <OdishaDestinationWorlds
          onExploreDestination={(id, name) => onNavigate('destinations', { query: name })}
          onPlanTrip={(name) => onNavigate('plan', { query: name })}
        />
      </div>

      {/* 3. V3 IMMERSIVE 3D HERITAGE SECTION */}
      <div id="heritage-3d-section">
        <Heritage3DSection
          onExplorePlace={(id, name) => onNavigate('destinations', { query: name })}
          onPlanTrip={(name) => onNavigate('plan', { query: name })}
        />
      </div>

      {/* 3. RESTORED DESTINATION DISCOVERY CAROUSEL */}
      <section
        data-testid="destination-discovery-section"
        className="w-full bg-[#FBF9F5] border-b border-[#E5DFD5] py-16 md:py-24 px-6 md:px-12"
      >
        <div className="max-w-7xl mx-auto space-y-8">
          <div className="max-w-3xl space-y-3">
            <span className="text-xs font-mono text-[#0D5C3A] tracking-widest uppercase font-semibold block">
              DESTINATION DISCOVERY
            </span>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-display font-bold text-[#12161E] tracking-tight leading-[1.15]">
              Places worth taking<br className="hidden sm:inline" /> the long way for.
            </h2>
            <p className="font-body text-sm sm:text-base text-[#70798B] leading-relaxed max-w-2xl">
              A curated look at Odisha's coast, temples, forests, lakes and historic cities across all 30 districts.
            </p>
          </div>

          {/* Glassmorphism Coverflow Carousel */}
          <CoverflowCarousel
            items={discoveryCarouselItems}
            onSelectItem={(item) => onNavigate('destinations', { query: item.title })}
            onExploreItem={(item) => onNavigate('destinations', { query: item.title })}
          />
        </div>
      </section>

      {/* 4. CONTINUOUS CIRCUITS TICKER */}
      <CircuitsTicker
        onSelectCircuit={(circuitName) => {
          onSearch(circuitName);
          onNavigate('destinations', { query: circuitName });
        }}
      />

      {/* 5. ESSENTIALS NEAR YOU SECTION */}
      <EssentialsSection
        onOpenHotels={() => onNavigate('map', { mode: 'hotels' })}
        onOpenMedical={() => onNavigate('map', { mode: 'medical' })}
        onOpenATM={() => onNavigate('map', { mode: 'atm' })}
        onOpenTransit={() => onNavigate('map', { mode: 'transit' })}
        onOpenCulinary={() => onNavigate('map', { mode: 'culinary' })}
        onOpenPetrol={() => onNavigate('map', { mode: 'petrol' })}
        onOpenPolice={() => onNavigate('map', { mode: 'police' })}
      />

      {/* 6. TRAVEL PERSONALITY QUIZ CALLOUT */}
      {onOpenOnboarding && (
        <section className="bg-[#FBF9F5] border-b border-[#E5DFD5] py-8 px-6 md:px-12">
          <div className="max-w-6xl mx-auto bg-white border border-[#E5DFD5] rounded-2xl p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6 shadow-xs">
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-full bg-[#0D5C3A]/10 text-[#0D5C3A] flex items-center justify-center shrink-0">
                <Sparkles className="w-6 h-6 text-[#C69214]" />
              </div>
              <div>
                <h3 className="font-display font-bold text-xl text-[#12161E]">
                  Personalize Your Odisha Expedition
                </h3>
                <p className="font-body text-xs md:text-sm text-[#70798B] mt-1">
                  Tell us your travel pace, culinary appetites, and passions to receive AI-curated itineraries.
                </p>
              </div>
            </div>

            <button
              onClick={onOpenOnboarding}
              className="px-6 py-3 bg-[#12161E] hover:bg-[#0D5C3A] text-white rounded-xl text-xs font-semibold transition-colors flex items-center gap-2 shrink-0 cursor-pointer shadow-xs"
            >
              <span>Take Personality Quiz</span>
              <ArrowRight className="w-4 h-4 text-[#C69214]" />
            </button>
          </div>
        </section>
      )}

      {/* 7. CURATED SIGNATURE EXPEDITIONS WITH 3D TILT CARDS */}
      <section className="py-20 px-6 md:px-12 max-w-7xl mx-auto w-full space-y-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-[#E5DFD5] pb-6">
          <div>
            <span className="text-xs font-mono text-[#0D5C3A] tracking-widest uppercase font-semibold">
              Editorial Signature Circuits
            </span>
            <h2 className="text-3xl md:text-4xl font-display font-bold text-[#12161E] mt-1">
              Curated Expeditions
            </h2>
          </div>
          <button
            onClick={() => onNavigate('destinations')}
            className="text-xs font-mono text-[#0D5C3A] hover:underline flex items-center gap-1 font-semibold cursor-pointer"
          >
            <span>View All 161 Destinations</span>
            <ArrowRight className="w-4 h-4 text-[#C69214]" />
          </button>
        </div>

        {/* 3 Major Signature Cards with 3D Tilt */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {/* Card 1: Konark & Golden Triangle */}
          <TiltCard
            onClick={() => onNavigate('plan', { hub: 'konark' })}
            className="group relative h-[450px] rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 cursor-pointer border border-[#E5DFD5]"
          >
            <img
              src={MANUAL_IMAGE_OVERRIDES["place_konark_001"] || getPlaceImageUrl("place_konark_001")}
              alt="Konark Sun Temple"
              className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#12161E] via-[#12161E]/40 to-transparent" />
            <div className="absolute bottom-8 left-8 right-8 text-white space-y-2">
              <span className="text-xs font-mono bg-[#0D5C3A] text-white px-3 py-1 rounded-full uppercase tracking-wider font-semibold">
                3 Days Classic
              </span>
              <h3 className="font-display text-2xl font-bold">
                The Golden Triangle
              </h3>
              <p className="font-body text-xs text-[#E5DFD5] line-clamp-2 leading-relaxed">
                Bhubaneswar's ancient rock edicts, Puri's sacred seaside rituals, and the stone sundials of Konark.
              </p>
            </div>
          </TiltCard>

          {/* Card 2: Chilika Marine Lagoon */}
          <TiltCard
            onClick={() => onNavigate('plan', { hub: 'chilika' })}
            className="group relative h-[450px] rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 cursor-pointer border border-[#E5DFD5]"
          >
            <img
              src="https://images.unsplash.com/photo-1589308078059-be1415eab4c3?auto=format&fit=crop&w=1200&q=80"
              alt="Chilika Lake"
              className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#12161E] via-[#12161E]/40 to-transparent" />
            <div className="absolute bottom-8 left-8 right-8 text-white space-y-2">
              <span className="text-xs font-mono bg-[#1B5E6B] text-white px-3 py-1 rounded-full uppercase tracking-wider font-semibold">
                2 Days Marine
              </span>
              <h3 className="font-display text-2xl font-bold">
                Chilika Water Sanctuaries
              </h3>
              <p className="font-body text-xs text-[#E5DFD5] line-clamp-2 leading-relaxed">
                Dolphin cruises on Asia's largest brackish lagoon, fresh seafood shacks, and Kalijai Island.
              </p>
            </div>
          </TiltCard>

          {/* Card 3: Koraput & Daringbadi */}
          <TiltCard
            onClick={() => onNavigate('plan', { hub: 'bhubaneswar' })}
            className="group relative h-[450px] rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 cursor-pointer border border-[#E5DFD5]"
          >
            <img
              src="https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=1200&q=80"
              alt="Daringbadi Valley"
              className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#12161E] via-[#12161E]/40 to-transparent" />
            <div className="absolute bottom-8 left-8 right-8 text-white space-y-2">
              <span className="text-xs font-mono bg-[#C69214] text-white px-3 py-1 rounded-full uppercase tracking-wider font-semibold">
                3 Days Highlands
              </span>
              <h3 className="font-display text-2xl font-bold">
                Misty Valleys of Kandhamal
              </h3>
              <p className="font-body text-xs text-[#E5DFD5] line-clamp-2 leading-relaxed">
                Pine forests, coffee plantations, and rolling clouds in the Eastern Ghats highlands.
              </p>
            </div>
          </TiltCard>
        </div>
      </section>
    </div>
  );
};
