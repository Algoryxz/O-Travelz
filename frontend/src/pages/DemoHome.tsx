...
export default function DemoHome() {
  ...
  const handleSurprise = () => {
    const open = NEARBY_PLACES.filter(p => p.open)
    setSurprise(open[Math.floor(Math.random() * open.length)])
    setActiveQuick('surprise')
  }

  const nearbyFiltered = (() => {
    const all = [...NEARBY_PLACES]
    if (nearbyTab === 'open')    return all.filter(p => p.open)
    if (nearbyTab === 'rated')   return [...all].sort((a,b) => b.rating - a.rating)
    if (nearbyTab === 'nearest') return [...all].sort((a,b) => a.distanceKm - b.distanceKm)
    return all
  })()
