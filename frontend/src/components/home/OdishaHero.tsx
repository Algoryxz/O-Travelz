    ...
    <section className="relative overflow-hidden bg-[#FBF9F5] border-b border-[#E5DFD5]">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-24">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          {/* Right Column: Curved/Stacked Glassmorphism Destination Panels */}
          <div className="lg:col-span-5 space-y-3">
            <div className="flex items-center justify-between text-xs text-[#70798B] px-1 font-mono uppercase tracking-wider">
              ...
              <span className="text-[#B87B22] font-semibold">{String(activeSlide + 1).padStart(2, "0")} / 04</span>
            </div>

            <div className="relative flex flex-col space-y-2.5 sm:space-y-3">
              {destinations.map((dest, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`w-full text-left p-3 sm:p-3.5 rounded-2xl transition-all duration-500 cursor-pointer flex items-center justify-between gap-3.5 backdrop-blur-[24px] backdrop-saturate-[135%] border select-none ${
                      isActive
                        ? "bg-white/[0.30] border-white/70 border-l-4 border-l-[#B87B22] shadow-[0_16px_40px_rgba(0,0,0,0.18)] lg:translate-x-2.5 scale-[1.02] z-20"
                        : "bg-white/[0.16] hover:bg-white/[0.24] border-white/35 hover:border-white/55 shadow-[0_8px_24px_rgba(0,0,0,0.1)] lg:translate-x-0 scale-[0.98] opacity-85 hover:opacity-100 z-10"
                    }`}
                  >
                    <div className="flex items-center gap-3.5 min-w-0">
                      {/* Destination Thumbnail / Photo Preview */}
                      <div className="relative w-12 h-12 rounded-xl overflow-hidden shadow-xs shrink-0 ring-1 ring-white/40">
                        <img
                          src={dest.bgImage}
                          alt={dest.name}
                        ...
                        />
                        <div className={`absolute inset-0 ${isActive ? "bg-[#B87B22]/15" : "bg-black/15"}`} />
                        {isActive && (
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="font-serif font-bold text-sm sm:text-base text-[#12161E] truncate">
                            {dest.name}
                          </span>
                          {dest.odiaName && (
                            <span className="text-[10px] text-[#70798B] font-odia truncate hidden sm:inline">
                              {dest.odiaName}
                            </span>
                          )}
                        </div>
                        <div className="text-[11px] text-[#3D4654] truncate mt-0.5">
                          {dest.category}
                        </div>
                        <div className="text-[10px] text-[#B87B22] font-mono flex items-center gap-1.5 mt-0.5">
                          <span className="w-1.5 h-1.5 rounded-full bg-[#B87B22] shrink-0" />
                          <span className="truncate">{dest.detail}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div
                      className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 transition-all ${
                        isActive
                          ...
                          : "bg-white/40 text-[#70798B] group-hover:bg-white/60"
                      }`}
                    >
                      <ChevronRight
                        size={14}
            ...
            <button
              ...
              className="w-full py-2.5 px-4 rounded-xl bg-white/90 hover:bg-white border border-[#E5DFD5] text-[#12161E] font-medium text-xs flex items-center justify-center gap-2 transition-all cursor-pointer shadow-xs hover:shadow-sm mt-1"
            >
