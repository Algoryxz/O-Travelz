...
export const SavedPlacesPage: React.FC<SavedPlacesPageProps> = ({
  ...
  return (
    <div
      ...
      className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in duration-300"
    >
      {/* Header */}
        ...
        {savedPlaces.length > 0 && (
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-md flex items-center gap-2 transition-colors cursor-pointer"
            >
              <Compass size={15} />
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-[#09221b] border border-emerald-800/50 hover:bg-emerald-900/50 text-emerald-200 font-bold text-xs shadow-xs flex items-center gap-2 transition-colors cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {savedPlaces.map((item) => (
              <div
                ...
                className="p-5 rounded-3xl bg-[#0a241d] border border-emerald-800/40 hover:border-emerald-500/50 shadow-md hover:shadow-xl transition-all flex flex-col justify-between gap-4"
              >
                <div>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    >
                      ...
                      <h3 className="font-display font-bold text-base text-white mt-0.5 hover:text-emerald-300 transition-colors">
                        {item.name}
                      </h3>
                    </div>
                    <button
                ...
                <div className="pt-3 border-t border-emerald-900/40 flex items-center justify-between text-xs text-gray-400">
                  <button
                    type="button"
                    onClick={() =>
                      onOpenMap({
                  ...
                  <span className="text-[11px] text-emerald-400/60 font-mono">{item.addedDate || "Saved"}</span>
                </div>
              </div>
            ))}
          </div>
