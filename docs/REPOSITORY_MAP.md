...
| `backend/app/schemas/map_projection.py` | Phase 6A internal projection schemas and public typed HTTP request schema with optional hop context | Rudra/Smarak/map owners | 6A | Map request/response shape, optional hop context, and internal authorization capability | Database lookup or projection business logic | Finalized Phase 6A HTTP V2 contract |
| `backend/app/api/places_routes.py` | Authoritative places discovery boundary (`GET /places`, `GET /places/{id}`) | Smarak/Rudra | 6B/Prod | Place listing, regional filtering, search, and place detail lookup | Direct UI rendering or external scraping | SQLAlchemy models |
| `backend/app/api/ai_routes.py` | Accepted `POST /ai/plan` conversational boundary | Smarak/Rudra | 5 | Request validation and orchestration wiring | Provider SDKs, deterministic business logic, direct database access | AI contracts and deterministic services |
| `backend/app/api/map_routes.py` | Accepted `POST /map/v1/projection` HTTP wiring | Rudra with map/core owners | 6A | Thin request/response adapter invocation | Projection logic, frontend behavior, persistence | Map HTTP adapter and projection contracts |
...
| `backend/app/ai/` | Accepted provider-neutral AI orchestration and grounding package | Smarak | 5 | Intent contracts, model boundary, orchestrator, grounding, and deterministic adapters | Deterministic facts, route logic, direct DB access, provider SDKs | Approved tools and contracts |
| `backend/app/ai/schemas.py` | AI intent, response, claim, and tool contracts | Smarak | 0/5 | Validated `AIResponse`, `AIIntent`, `ModelResponse`, finite framing, and approved tool-call types | Model execution or factual constants | Deterministic service contracts |
| `backend/app/ai/model.py` | Provider-neutral model adapter protocol and deterministic implementations | Smarak | 5 | `ModelAdapter`, `FakeModelAdapter`, narrow offline `RuleBasedModelAdapter` | Commercial provider integration | AI contracts and grounding boundary |
| `backend/app/ai/orchestrator.py` | Accepted AI orchestration boundary | Smarak | 5 | Intent validation, constraint refinement, approved tool execution, AI status/error behavior | Ranking, routing, itinerary mutation, direct DB access | Deterministic services and AI tools |
| `backend/app/ai/grounding.py` | Accepted current-turn grounding boundary | Smarak | 5 | Fact registry, deep-copied model snapshot, exact claim/value validation, deterministic rendering | Model factual research or free-form factual prose | Deterministic tool results |
| `backend/app/ai/tools/` | Accepted deterministic AI tool adapters | Smarak | 5 | `build_itinerary`, `plan_transport_hop`, and `get_provider_status` wrappers | Provider SDK clients or business logic | Ranking, itinerary, transport services |
| `backend/app/geospatial/` | Geospatial preparation package; remaining Phase 6A map representation remains gated while the bounded HTTP adapter is accepted | Susmita for canonical geospatial behavior; Smarak temporarily coordinates Phase 6A execution; Rudra for routing outputs | 3/6A | Contract-independent validation and approved bounded HTTP geometry/map representations | Complete frontend, routing, AMA coordinate mapping, AMA route geometry, or invented geometry | Transport and map contracts; current Phase 6A reconciliation |
...
| `backend/tests/test_geospatial_validation.py` | Phase 6A preparation tests for supplied geometry and current transport contract semantics | Susmita | 6A preparation | Coordinate/LineString validation, NULL preservation, ordered synthetic legs, tier/provider/unavailable preservation | Routing, geometry inference, AMA/GIS identity mapping, public map contract | `backend/app/geospatial/validation.py`, fixture cases |
| `backend/tests/test_phase6a_map_projection.py` | Phase 6A projection-core contract tests | Map/core owners | 6A | Core identity, geometry, relationship, and exclusion tests | HTTP binding or persistence | `backend/app/geospatial/projection.py`, map schemas |
| `backend/tests/test_phase6a_map_http.py` | Phase 6A HTTP boundary tests (V1 baseline + V2 hop context) | Rudra with map/core owners | 6A | Endpoint, exact typed UUID binding, optional hop context, errors, unavailable coverage, and redaction | Frontend, persistence, provider changes | `backend/app/api/map_routes.py`, HTTP adapter |
...
| `docs/handoffs/2026-08-18_SMARAK_PHASE5_SCOPE_REPORT.md` | Accepted Phase 5 scope and limitations | Smarak | 5 | Grounded AI boundary, approved tools, refinements, and exclusions | Phase 6 implementation or provider selection | Phase 5 acceptance handoff |
| `docs/handoffs/2026-08-18_SMARAK_PHASE5_IMPLEMENTATION_HANDOFF.md` | Accepted Phase 5 implementation evidence | Smarak | 5 | Files, contracts, grounding, tests, limitations, and provider-neutral boundary | Provider integration or Phase 6 implementation | Phase 5 acceptance handoff |
| `docs/handoffs/2026-08-18_SMARAK_PHASE5_ACCEPTANCE_HANDOFF.md` | Official Phase 5 acceptance checkpoint | Smarak/Punam | 5 | Final verdict, date, architecture, ownership, evidence, limitations, and next-phase readiness | New feature implementation or silent provider decision | Canonical docs and Phase 5 implementation |
| `docs/handoffs/2026-08-18_SMARAK_PHASE6A_RESEARCH_CLOSURE_RECONCILIATION.md` | Current Phase 6A research closure and canonical state reconciliation | Smarak/Punam | 6A | Official GIS evidence, absent crosswalk, reduced scope, temporary ownership overlay, blockers, and next sequence | Phase 6A implementation, coordinate promotion, route geometry, or historical report rewriting | Canonical docs, AMA research handoffs, official GIS services |
| `docs/handoffs/2026-08-18_CODEX_PHASE6A_HTTP_IMPLEMENTATION.md` | Bounded Phase 6A accepted HTTP implementation handoff | Backend/API implementation task | 6A | Endpoint files, contract boundaries, tests, scope verification, and next verification gate | Claiming full Phase 6A completion or frontend/map UI | Accepted HTTP contract, projection core, backend suite |
| `docs/handoffs/2026-08-18_CODEX_PHASE6A_HTTP_CLOSEOUT.md` | Phase 6A HTTP acceptance and closeout handoff | Backend/API implementation task | 6A | Final acceptance, verification evidence, safety boundaries, and remaining bounded scope | Claiming full Phase 6A completion or frontend/map UI | Accepted HTTP contract, projection core, final verification |
| `docs/handoffs/2026-08-18_SMARAK_PHASE6A_REDUCED_MAP_CONTRACT_V2.md` | Finalized Phase 6A Reduced Map Contract V2 | Smarak/map owners | 6A | Four approved human decisions, bounded HTTP extension, hop context trust boundary, and geometry authority | Unapproved geometry inference or RouteStop | Accepted Phase 6A HTTP V1 and projection core |
| `docs/handoffs/2026-08-18_CODEX_PHASE6A_HTTP_V2_IMPLEMENTATION.md` | Phase 6A HTTP V2 implementation handoff | Backend/API implementation task | 6A | Implementation files, optional hop context, V1 backward compatibility, and test counts | Claiming full Phase 6A completion | Finalized V2 contract and backend suite |
| `docs/handoffs/2026-08-18_CODEX_PHASE6A_HTTP_V2_CLOSEOUT.md` | Phase 6A HTTP V2 acceptance and closeout handoff | Backend/API implementation task | 6A | Final acceptance, verification evidence, hop/leg behavior, and remaining bounded scope | Claiming full Phase 6A completion or frontend/map UI | Finalized V2 contract, projection core, final verification |
| `backend/tests/test_ai_phase5.py` | Phase 5 deterministic transcript and grounding tests | Smarak | 5 | 19 focused tests for orchestration, grounding, refinements, errors, and API envelope | Production provider behavior or frontend UX | AI implementation and deterministic services |
