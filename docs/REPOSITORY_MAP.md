...
| `backend/app/schemas/transport.py` | Transport hop and provider-status schemas | Rudra/Smarak | 0/3 | Transport contract types | Provider adapters or provider facts | Transport contract |
...
| `backend/app/api/` | Existing empty API package | Rudra | 4–7 | `TO CREATE`: API routers and HTTP wiring | Ranking or AI internals | Backend service contracts |
| `backend/app/services/ranking/` | Existing empty ranking package | Smarak | 4 | `TO CREATE`: deterministic ranking | Provider integrations or AI | Database/place data |
| `backend/app/services/itinerary/` | Existing empty itinerary package | Smarak | 4 | `TO CREATE`: deterministic itinerary generation | Frontend or provider adapters | Ranking and transport service |
...
| `backend/app/geospatial/` | Existing empty geospatial package | Susmita for geospatial behavior; Rudra for backend routing outputs | 3/6A | `TO CREATE`: approved geometry/map representations | Complete frontend or invented geometry | Transport and map contracts |
