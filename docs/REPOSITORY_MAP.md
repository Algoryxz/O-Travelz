...
| `backend/app/schemas/map_projection.py` | Phase 6A internal projection schemas and public typed HTTP request schema | Rudra/Smarak/map owners | 6A | Frozen map request/response shape and internal authorization capability | Database lookup or projection business logic | Accepted Phase 6A HTTP contract |
...
| `backend/app/geospatial/http_adapter.py` | Exact typed UUID binding and backend-geometry adaptation for the accepted map endpoint; HTTP slice accepted 2026-08-18 | Rudra with map/core owners | 6A | Place/Stop/Route lookup, authorized backend refs, core invocation, structured map errors | Name/provider/GIS matching, client geometry, hop or RouteStop authority | Existing models, geometry primitives, projection core |
...
| `backend/tests/test_phase6a_map_http.py` | Phase 6A accepted HTTP boundary tests | Rudra with map/core owners | 6A | Endpoint, exact typed UUID binding, errors, unavailable coverage, and redaction | Frontend, persistence, provider changes | `backend/app/api/map_routes.py`, HTTP adapter |
