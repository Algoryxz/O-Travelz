...
class TestSessionManagerLifecycle:
    def test_create_and_verify_active_session(self, unit_db: Session):
        db = unit_db
        user = User(
            id=uuid.uuid4(),
    ...
    def test_expired_session_rejected(self, unit_db: Session):
        db = unit_db
        user = User(
            id=uuid.uuid4(),
            email=f"traveler_{uuid.uuid4()}@example.com",
    ...
    def test_revoked_session_rejected(self, unit_db: Session):
        db = unit_db
        user = User(
            id=uuid.uuid4(),
            email=f"traveler_{uuid.uuid4()}@example.com",
    ...
    def test_user_resolution_and_profile_upsert(self, unit_db: Session):
        db = unit_db
...
class TestAuthAPIEndpoints:
    ...
    def test_get_me_authenticated_with_valid_cookie(self, unit_db: Session):
        db = unit_db
        user = User(
            id=uuid.uuid4(),
    ...
    def test_logout_endpoint_clears_cookie_and_revokes_session(self, unit_db: Session):
        db = unit_db
        user = User(
            id=uuid.uuid4(),
    ...
    def test_get_me_with_bearer_authorization_header(self, unit_db: Session):
        db = unit_db
        user = User(
            id=uuid.uuid4(),
    ...
    def test_verify_session_with_quoted_and_whitespace_token(self, unit_db: Session):
        db = unit_db
        user = User(
            id=uuid.uuid4(),
