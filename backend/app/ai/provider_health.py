...
def inspect_provider_health(
    ...
    if name == "groq":
        ...
        base_url = getattr(cfg, "ai_groq_api_base_url", None) or getattr(cfg, "groq_base_url", None) or "https://api.groq.com/openai/v1"
        model = getattr(cfg, "ai_groq_model_name", None) or getattr(cfg, "groq_model", None) or "llama-3.3-70b-versatile"
        configured = bool(api_key and base_url)

        if not configured:
            state = ProviderReadinessState.NOT_CONFIGURED
            allowed = False
            details = "Missing GROQ_API_KEY for Groq Cloud."
        elif not externally_enabled:
            state = ProviderReadinessState.CONFIGURED_BUT_DISABLED
            allowed = False
            details = "Configured but blocked: AI_ALLOW_EXTERNAL_PROVIDER is false."
...
def inspect_all_providers(settings: Optional[Settings] = None) -> list[ProviderHealthStatus]:
    ...
    return [
        ...
        inspect_provider_health("groq", settings),
