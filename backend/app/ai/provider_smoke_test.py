...
def execute_single_provider_smoke_test(
    ...
    if provider_name == "azure_openai":
        ...
        adapter = GroqProviderAdapter(
            api_base_url=getattr(cfg, "ai_groq_api_base_url", None) or getattr(cfg, "groq_base_url", None) or "https://api.groq.com/openai/v1",
            api_key=getattr(cfg, "ai_groq_api_key", None) or getattr(cfg, "groq_api_key", None),
            model_name=getattr(cfg, "ai_groq_model_name", None) or getattr(cfg, "groq_model", None) or "llama-3.3-70b-versatile",
            vision_model_name=getattr(cfg, "ai_groq_vision_model_name", "llama-3.2-11b-vision-preview"),
            timeout_seconds=min(10.0, getattr(cfg, "ai_timeout_seconds", 10.0)),
            max_retries=0,
        )
