...
class NVIDIAProviderAdapter(GenericHTTPProviderAdapter):
    ...
    def __init__(
        ...
        super().__init__(
            api_base_url=api_base_url or "https://integrate.api.nvidia.com/v1",
            api_key=api_key,
            model_name=model_name or "meta/llama-3.1-8b-instruct",
            timeout_seconds=timeout_seconds,
            max_retries=max_retries,
...
class MultiProviderFallbackAdapter(AIProviderAdapter):
    """Zero-cost priority router mediating Azure OpenAI -> Gemini -> NVIDIA -> RuleBased fallback.
...
def create_provider_adapter(settings: Any | None = None) -> AIProviderAdapter:
    ...
    - 'multi_provider' -> MultiProviderFallbackAdapter (Azure -> Gemini -> NVIDIA -> RuleBased)
    ...
    nvidia_adapter = NVIDIAProviderAdapter(
        ...
        api_key=getattr(settings, "ai_nvidia_api_key", None),
        model_name=getattr(settings, "ai_nvidia_model_name", "meta/llama-3.1-8b-instruct"),
        timeout_seconds=getattr(settings, "ai_timeout_seconds", 30.0),
        max_retries=getattr(settings, "ai_max_retries", 2),
    )

    # Multi-provider fallback chain (Azure -> Gemini -> NVIDIA)
    if provider_name in ("multi_provider", "azure_openai", "gemini", "nvidia"):
        if not allow_external:
            ...
            return MultiProviderFallbackAdapter(
                providers=[azure_adapter, gemini_adapter, nvidia_adapter],
        ...
        if provider_name == "azure_openai":
            chain = [azure_adapter, gemini_adapter, nvidia_adapter]
        elif provider_name == "gemini":
            chain = [gemini_adapter, azure_adapter, nvidia_adapter]
        elif provider_name == "nvidia":
            chain = [nvidia_adapter, azure_adapter, gemini_adapter]
        else:
            chain = [azure_adapter, gemini_adapter, nvidia_adapter]
    ...
    raise UnsupportedCapabilityError(
        f"Unsupported AI provider: '{provider_name}'. Supported providers: mock, rule_based, azure_openai, gemini, nvidia, openai_compatible, multi_provider.",
