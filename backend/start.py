    ...
    try:
        ...
        if res.returncode != 0:
            print(
                ...
                f"Proceeding with startup since database schema may already be initialized.",
                file=sys.stderr,
                flush=True,
            )
        else:
        ...
        print(
            f"[STARTUP WARNING] Database migration timed out after 60s: {exc}. "
            f"Proceeding with server startup.",
            file=sys.stderr,
            flush=True,
        )
